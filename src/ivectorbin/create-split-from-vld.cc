...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "on a VLD (voiced language detection) file.\n"
        "The VLD file is the output of merge-vad-with-frame-labels or a similar program "
        "(a vector \n"
        "of length num-frames, containing 1.0 for target language/spkr, 0.0 for unvoiced, \n"
    	"and -ve integers for languages/spkrs we want to prune."
        "Each line of the\n"
        ...
        "E.g.: create-split-from-vld [options] scp:vad.scp feats_segment\n";
    ...
    for (;!vad_reader.Done(); vad_reader.Next()) {
      ...
      for (int32 j = 0; j < vad.Dim(); j++) {
    	...
    	// if you do not want to count the sil regions towards the length of segment,
    	// where length of segment is measured by "curr_sum_voiced_frames",
    	// then comment the else if condition below
    	// else if (vad(j) == 0) {
    	//	curr_sum_voiced_frames++;
    	// }
