...
if [ $# != 2 ]; then
  echo "Usage: $0 /path/to/music-wav  data/ directory"
  exit 1; 
fi 
...
find "$WAVROOT" -type f -iname "*.wav" > $tmpdir/music_wav.flist

[ -d "$data" ] || mkdir -p $data;
# prepare wav.scp, spk2utt, utt2spk, spk2gender files
local/make_music.pl $tmpdir/music_wav.flist $data

echo "music data prep succeeded".
exit 0;
