...
segwav_dir=$corpus_dir/www.sbs.com.au/seg # dir where you want to store the segmented wav files
...
# target languages present in www.sbs.com.au/ 
...
do
{
	echo "Starting language id for language=$l";
	t1=$(date +"%s");
	sid/language_id.sh --cmd "$train_cmd" --nj 1 --tlang-prior $tlang_prior --olang1-prior $olang1_prior --olang2-prior $olang2_prior \
...
for l in `echo $desired_lang`
do
local/extract_segments.sh --cmd "$train_cmd" --nj 1 --max-seg-length $max_seg_length data/train_${l} $segwav_dir exp/language_id_train_${l}
done  
fi
